...
function [drDelay, solarDelay, pmuDelay,...
    ...
    extractResults(nRuns, dummy, beta, d2dScheduler, explTime)
...
for r = 1:nRuns
    logfileName = strcat('./Results/', num2str(neNBs), 'eNBs_', num2str(nD2D),...
        'D2Ds_', num2str(dummy), 'Dummy_', num2str(nUEs), 'UEs_', d2dScheduler, '_Run', num2str(r),...
...
for r = 1:nRuns
    convfileName = strcat('./Results/convLogFile_', num2str(neNBs), 'eNBs_', num2str(nD2D),...
        'D2Ds_', num2str(dummy), 'Dummy_', num2str(nUEs), 'UEs_', d2dScheduler, '_Run', num2str(r),...
        '_Beta', num2str(beta), '.dat');
...
% CQI average value
dr2CQI = mean(dr2AvgCQI, 1);
dr3CQI = mean(dr3AvgCQI, 1);
dr4CQI = mean(dr4AvgCQI, 1);
drCQI = mean([dr2CQI, dr3CQI, dr4CQI], 1);
solarCQI = mean(solarAvgCQI, 1);
pmuCQI = mean(pmuAvgCQI, 1);
