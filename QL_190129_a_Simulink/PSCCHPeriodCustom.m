% PSCCHPeriod PSCCH period for ProSe sidelink direct communications
%   Transmission and reception opportunities for sidelink direct
%   communications are associated with a set of periodically occurring
%   time-domain periods known as PSCCH periods. The overall structure of a
%   PSCCH period is defined by the SL-CommResourcePool RRC signalling
%   message which contains PSCCH and PSSCH resource configuration parts. 
%   A UE can be configured with multiple messages and therefore multiple
%   PSCCH periods.
% 
%   The semi-static SL-CommResourcePool RRC message (TS 36.331
%   Section 6.3.8) defines the length of the PSCCH period and the subsets
%   of subframes and resource blocks within the period, known as pools,
%   that can be used for PSCCH and PSSCH transmission and reception. When
%   data transmission is to occur within a given PSCCH period, the actual
%   resources are selected from the subframe and resource block pools
%   using dynamic scheduling information. The scheduling happens on a
%   per-period basis using the transmission grant provided by the eNodeB
%   (using a DCI format 5 message) when the UE is in-coverage and in
%   transmission mode 1, or selected directly by the UE when
%   out-of-coverage or in transmission mode 2. The scheduling information
%   associated with the PSSCH transmission is sent on the PSCCH within
%   the PSCCH period using an SCI format 0 message. The procedures, which
%   describe how the RRC configuration and DCI/SCI messages are combined
%   to give transmission resources, are defined in TS 36.213 Sections 14.1
%   and 14.2.
%
%   PSCCHPeriod models a single PSCCH period configured by a structure
%   representing the SL-CommResourcePool RRC message, along with additional
%   general parameters. The class can combine these semi-static RRC
%   parameters with dynamic DCI/SCI parameters to define a specific
%   transmission sequence. A PSCCHPeriod object can generate a baseband
%   waveform for the PSCCH period which transmits a pair of PSCCH carrying
%   SCI format 0, followed by the associated sequence of PSSCH carrying
%   SL-SCH encoded transport block transmissions. The waveform can also
%   include a sequence of sidelink synchronization subframes carrying
%   PSSS/SSSS/PSBCH. PSCCHPeriod can also display an image representing the
%   PSCCH period resource grid, indicating the subframe and resource block
%   pools alongside the specific PSCCH and PSSCH transmission instance.
%   The pools are given by vectors of 0-based indices, relative to the 
%   PSCCH period. The class also provides information about the location of
%   the pools and other procedural information defined in TS 36.213 
%   Sections 14.1 and 14.2. Note that PSCCHPeriod is a handle class so use
%   the copy function to create a copy of an object.
%
%   P = PSCCHPeriod() creates a PSCCH period object P from a default 
%   resource configuration. This configuration parameter set is the same 
%   as that returned by the PSCCHPeriod.defaultConfig() static method.
% 
%   P = PSCCHPeriod(C) creates a PSCCH object given the parameter
%   structure C. This input structure is based on the RRC information
%   element, SL-CommResourcePool, along with additional general parameters.
%   An instance of the parameter structure can be created by the 
%   PSCCHPeriod.defaultConfig() static method. The parameter structure
%   associated object P can be accessed and directly modified using
%   property <a href="matlab: help('PSCCHPeriod/Config')">Config</a>.
%   
%   PSCCHPeriod properties:
%
%   PSCCH pool information
%   PSCCHSubframePool      - PSCCH subframe pool indices (0-based). Read-only
%   PSCCHResourceBlockPool - PSCCH resource block pool indices (0-based). Read-only
%   NumPSCCHResource       - Number of allowed nPSCCH ("Resource for PSCCH") values. Read-only
%
%   PSSCH pool information
%   PSSCHSubframePool      - PSSCH subframe pool indices (0-based). Read-only
%   PSSCHResourceBlockPool - PSSCH resource block pool indices (0-based). Read-only
%   AllowedITRP            - Allowed ITRP values. Read-only
%
%   PSSS/SSSS/PSBCH synchronization subframe information
%   SyncSubframes          - Synchronization subframe indices (0-based). Read-only
%
%   Configuration parameters
%   Config                 - RRC resource pool and general parameters
%
%   PSCCHPeriod methods:
%
%   generateWaveform  - Generate baseband waveform of PSCCH/PSSCH/Sync transmission
%   displayPeriod     - Display image of PSCCH period resource grid
%   getPSCCHResources - Get PSCCH transmission resources given DCI/SCI
%   getPSSCHResources - Get PSSCH transmission resources given DCI/SCI
%   getAllowedRIV     - Get allowed RIV values given DCI/SCI
%   resetConfig       - Reset configuration parameters to the default
%   defaultConfig     - Create default configuration parameter structure (static)
%   copy              - Copy the PSCCHPeriod handle object
%
%   % Examples:
%   % Display an image of the PSCCH and PSSCH resource pools defined within
%   % an example of a PSCCH period.
%
%   period = PSCCHPeriod;
%   displayPeriod(period);
%
%   % Reconfigure the object with a different pre-defined set of parameters
%   % and display the resource pools in the resource block grid of the
%   % period.
% 
%   period.Config = PSCCHPeriod.defaultConfig(1,'10MHz');
%   displayPeriod(period);
%
%   % Define a sequence of PSCCH and PSSCH transmissions within a PSCCH
%   % period. The PSSCH sequence uses a single PRB allocation and type 2
%   % hopping. This is done via the allocation parts of a DCI format 5 
%   % message. Display an image of the active resource blocks/subframe 
%   % used within the PSCCH and PSSCH resource pools. Generate the 
%   % associated resource element grid and baseband waveform and display 
%   % both of them.
%
%   period = PSCCHPeriod;
%   dci.PSCCHResource = 0;
...
%   dci.FreqHopping = 1; 
%   dci.Allocation.HoppingBits = 1; 
%   dci.Allocation.RIV = 0;
%   displayPeriod(period,dci);
%   [waveform,grid] = generateWaveform(period,dci);
%   figure, image(abs(grid)*100), axis('xy'), title('Resource elements');
%   figure, plot(abs(waveform)), title('PSCCH period baseband waveform');
%   
%   See also ltePSSS, ltePSSSIndices, lteSSSS, lteSSSSIndices,
%   lteSLMIB, lteSLBCH, ltePSBCH, ltePSBCHIndices, ltePSBCHDRS,
%   ltePSBCHDRSIndices, lteSCI, lteSCIEncode, lteSCIResourceAllocation,
%   ltePSCCH, ltePSCCHIndices, ltePSCCHDRS, ltePSCCHDRSIndices, lteSLSCH,
%   lteSLSCHDecode, ltePSSCH, ltePSSCHIndices, ltePSSCHDRS,
%   ltePSSCHDRSIndices, lteSLResourceGrid, lteSLResourceGridSize,
%   lteSLSCFDMAModulate, V2XSidelinkResourcePool.

%   Copyright 2016-2017 The MathWorks, Inc.

classdef PSCCHPeriodCustom < matlab.mixin.Copyable
    ...
    properties (Access = public, SetObservable = true)
        ...
        % Config Configuration parameters for PSCCH period resource pool
        %   This property is a structure containing all the semi-static parameters
        %   which configure the PSCCH period. It can be set in the constructor
        %   and/or any of the individual parameters can be modified later.
        %   
        %   It is a combination of general link settings and parameters from the
        %   SL-CommResourcePool-r12 RRC (layer 3) message. This message configures
        %   a single transmit or receive pool for sidelink direct communications.
        %   The structure also includes parameters from a single SL-SyncConfig-r12
        %   message which allows configuration of a synchronization and broadcast
        %   channel subframe sequence. In the full LTE layer 3 protocol, these 
        %   sync parameters are transmitted as part of a list in SIB18 (if 
        %   available) and referenced by index from the SL-CommResourcePool-r12 
        %   messsage. All the parameter names that end with the suffix '_r12' 
        %   map onto information elements from TS 36.331 Section 6.3.8 - Sidelink
        %   information elements.
        %
        %   The Config property structure should contains the following fields:
        %
        %   NSLRB           - Number of sidelink resource blocks (6...110)
        %   DuplexMode      - Duplex mode ('FDD','TDD')
        %   TDDConfig       - TDD Uplink/Downlink Configuration (0...6)
        %   UESelected      - Enable UE-selected transmission mode 2 ('on','off')
        %   SyncEnable      - Enable synchronization subframes ('on','off')
        %   NPSCCHPeriod    - PSCCH period number
        %   sc_CP_Len_r12   - PSCCH cyclic prefix ('Normal','Extended')
        %   sc_Period_r12   - PSCCH period length (40,60,70,80,120,140,160,240,280,320)
        %   sc_TF_ResourceConfig_r12 - Substructure (SL-TF-ResourceConfig-r12)
        %       prb_Num_r12           - Number of PRB in one pool band (0...100)
        %       prb_Start_r12         - PRB start index (0...99)
        %       prb_End_r12           - PRB end index (0...99)
        %       offsetIndicator       - PSCCH offset indicator (large-r12, 0..10239)
        %       subframeBitmap_r12    - PSCCH subframe bitmap (4,8,12,16,30,40,42 bits - char vector or numerical vector)
        %   data_CP_Len_r12 - PSSCH cyclic prefix ('Normal','Extended')
        %   dataHoppingConfig_r12    - Substructure (SL-TF-ResourceConfig-r12)
        %       hoppingParameter_r12  - PSSCH Hopping parameter (0...504...510) 
        %       numSubbands_r12       - Number of subbands (1,2,4)
        %       rb_Offset_r12         - PSSCH hopping offset (0...110)
        %   ue_SelectedResourceConfig_r12 - Substructure
        %       data_TF_ResourceConfig_r1  - Substructure (SL-TF-ResourceConfig-r12)
        %           prb_Num_r12             - Number of PRB in one pool band (0...100)
        %           prb_Start_r12           - PRB start index (0...99)
        %           prb_End_r12             - PRB end index (0...99)
        %           offsetIndicator_r12     - PSSCH offset indicator (short-r12, 0..319)
        %           subframeBitmap_r12      - Bitmap of length 4,8,12,16,30,40,42 bits
        %       trpt_Subset_r12            - T-RPT subset bitmap (3...5 bits)
        %   syncConfig - Substructure (SL-SyncConfig-r12)
        %       syncCP_Len_r12            - Sync cyclic prefix ('Normal','Extended')
        %       syncOffsetIndicator_r12   - Sync offset indicator (0...39)
        %       slssid_r12                - Sidelink synchronization identity (0...167...335)
        %
        %   Note that this parameter structure has a similar hierarchy to the 
        %   actual SL-CommResourcePool-r12 message (from TS 36.331 Section 6.3.8):
        %
        ...
        %   }
        %
        %   There are further restrictions placed on the RRC message parameter
        %   combinations, for example, bitmap lengths for different TDD configurations, 
        %   as defined in TS 36.331, but these are are not enforced in this class.
        % 
        %   Examples:
        %   % Display the parameters held in the Config structure used in 
        %   % a default PSCCHPeriod object
        %   period = PSCCHPeriod;
        %   period.Config
        %   period.Config.sc_TF_ResourceConfig_r12
        %   period.Config.dataHoppingConfig_r12
        %   period.Config.dataHoppingConfig_r12
        %   period.Config.ue_SelectedResourceConfig_r12
        %   period.Config.syncConfig
        %
        %   % Display the period object's properties and resourcing before and
        %   % after changing from FDD to TDD duplexing. Note the effect on the  
        %   % PSCCH and PSSCH subframe pool properties.
        ...
        %   period.displayPeriod();
        %
        %   See also defaultConfig, PSCCHPeriod, V2XSidelinkResourcePool.
        Config;
    ...
    methods (Access = public, Static=true)
        ...
        %defaultConfig Create default configuration parameter structure (static)
        %   P = defaultConfig(CID,BW) returns a pre-defined parameter 
        %   structure P that can be used to construct or re-configure a PSCCHPeriod
        %   object. The parameters are based on the reference single pool 
        %   configurations given in TS 36.101 Section A.7.2, specifically pool 
        %   configuration #1-FDD (Table A.7.2.1-1), #2-FDD (Table A.7.2.1-2) and 
        %   #5-FDD (Table A.7.2.1-5). The scalar input parameter CID selects the
        %   pool configuration and must be 1, 2 or 5. BW defines the transmission 
        %   bandwidth and must be one of '1.4MHz','3MHz','5MHz','10MHz','15MHz',
        %   '20MHz'. Both parameters are optional and default to configuration #5
        %   and 5MHz respectively if absent.
        % 
        %   All these cases are FDD and are defined for UE-selected or mode 2 
        %   transmission.  Configuration #1 is intended for out-of-network coverage
        %   operation and has synchronization subframe transmission enabled. The
        %   resource block pools (PSCCH and PSSCH) are both full band and the 
        %   period length is 40ms. The configurations defined in Section A.7.2 are
        %   specifically for 5MHz or 10MHz but this function allows for any of the
        %   above transmission bandwidths.
        %
        %   Examples: 
        %   % Create pre-defined parameter configurations, use them to configure
        %   % PSCCH periods and display the layout of the associated subframe and
        %   % resource block pools.
        %   
        %   % Construct a default PSCCH period object and show that the object's 
        %   % configuration is the same as for configuration #5 (5MHz).
        %   period = PSCCHPeriod();
        %   config = PSCCHPeriod.defaultConfig(5);
        %   isequal(config,period.Config)
        %   period.displayPeriod();   
        %
        %   % Re-configure the PSCCH period object with configuration #1 (5MHz).
        %   config = PSCCHPeriod.defaultConfig(1);
        %   period.Config = config;
        %   period.displayPeriod();
        %
        %   % Change the frame type to TDD and display the change in the subframe
        %   % pools due to the combination of uplink and downlink subframes.
        %   period.Config.DuplexMode = 'TDD';
        %   period.displayPeriod();
        %   
        %   % Construct a PSCCH period object with configuration #2 (10MHz).
        %   config = PSCCHPeriod.defaultConfig(2,'10MHz');
        %   period = PSCCHPeriod(config);
        %   period.displayPeriod();
        %   
        %   % Construct a PSCCH period object with configuration #5 (10MHz).
        %   period = PSCCHPeriod(PSCCHPeriod.defaultConfig(5,'10MHz'));
        %   period.displayPeriod();
        %
        %   See also Config, displayPeriod.
    ...
    methods
        ...
        % Class constructor
        function obj = PSCCHPeriod(params)
        ...
        %% generateWaveform
        function [waveform,grid] = generateWaveform(obj, dci, TBs)
        %generateWaveform Generate baseband waveform for PSCCH period
        %   [waveform,grid] = generateWaveform(obj,dci) creates a modulated
        %   baseband waveform for the PSCCH period given the DCI/SCI scheduling
        %   grant. The waveform contains the PSCCH pair carrying the associated
        %   SCI format 0 message payload, the scheduled PSSCH sequence, and any
        %   synchronization subframes present. The output grid is an
        %   NRE-by-NS matrix containing the complex modulation symbols for all the
        %   resource elements across all the SC-FDMA symbols in the subframes of
        %   the period. The resource elements of the last SC-FDMA guard symbol are
        %   not zeroed since this is carried out by the sidelink SC-FDMA modulation
        %   stage. Note that cyclic prefix associated with the concatenated
        %   subframes can vary depending on the type of the subframe (PSCCH, PSSCH
        %   or sync) and so the start of the subframes may not be equally spaced
        %   within the grid matrix. All the available PSSCH transmissions are used
        %   and the data for each new transport block is chosen at random and sent 
        %   four times using the fixed RV sequence, RV = 0,2,3,1.
        %
        %   The input dci must be a structure containing the fields:
        %   PSCCHResource       - PSCCH resource index ("Resource for PSCCH"). Optional.
        %   ModCoding           - PSSCH modulation and coding index 
        ...
        %
        %   Note that the modulation and coding scheme must be passed in as an
        %   input to the function along with the other dynamic resource allocation
        %   parameters. In the full LTE protocol it is possible for this value to 
        %   be signalled via RRC messaging.
        %
        %   The antenna ports used by the various channels are defined as 
        %   port 1000 for PSCCH and PSCCH, port 1010 for PSBCH and 
        %   port 1020 for PSSS/SSSS. This function combines all into a single
        %   output grid and waveform.
        % 
        %   Example:
        %   % Generate a baseband waveform containing a sequence of matching 
        %   % PSCCH and PSSCH transmissions. Plot the waveform and display an 
        %   % image of the resource element grid for the entire period.
        %
        %   period = PSCCHPeriod;  
        %   dci.PSCCHResource = 0;
        %   dci.TimeResourcePattern = 106; 
        ...
        %   plot(abs(waveform));
        %   figure;
        %   image(100*abs(grid)); axis xy;
        %   figure;
        %   period.displayPeriod(dci);
        %
        %   See also displayPeriod, lteDCI, ltePSSS, lteSSSS, lteSLMIB, lteSLBCH,
        %   ltePSBCH, lteSCI, lteSCIEncode, lteSCIResourceAllocation, ltePSCCH,
        %   lteSLSCH, ltePSSCH, lteSLResourceGridSize, lteSLSCFDMAModulate.
